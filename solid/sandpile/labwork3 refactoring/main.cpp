...
Color getColor(uint64_t sand) {
    if (sand == 0) return { 255, 255, 255 };
    if (sand == 1) return { 0, 255, 0 };
    if (sand == 2) return { 128, 0, 128 };
    if (sand == 3) return { 0, 255, 255 };
    return { 0, 0, 0 };
}

class ISandTopplingStrategy {
public:
    virtual void topple(std::vector<std::vector<uint64_t>>& grid) = 0;
    virtual ~ISandTopplingStrategy() = default;
};

class ClassicToppling : public ISandTopplingStrategy {
public:
    void topple(std::vector<std::vector<uint64_t>>& grid) override {
