...
def binary_search(arr: List[int], target: int) -> Optional[int]:
    ...
    # O(log n) - цикл уменьшает диапазон в 2 раза на каждой итерации
    while left <= right:
...
def measure_time(
    func, arr: List[int], target: int, number_of_runs: int = 100
) -> float:
    """
    ...
    total_time = timeit.timeit(
        lambda: func(arr, target), number=number_of_runs
    )
    # Конвертация в миллисекунды
    return (total_time / number_of_runs) * 1000
...
def main():
    ...
    for size in sizes:
        ...
        print(f"{size:6} | {linear_time:12.4f} | "
              f"{binary_time:12.4f} | {ratio:8.1f}x")
    ...
    last_size = sizes[-1]
    speed_ratio = linear_times[-1] / binary_times[-1]
    print(f"   - Для {last_size} элементов: в {speed_ratio:.1f} раз быстрее")

    print("\n4. Визуальное подтверждение:")
    print("   - График в линейном масштабе: виден экспоненциальный рост "
          "разницы")
    print("   - График в log-log scale: линейный поиск - прямая линия")
    print("     (линейный рост), бинарный - пологая кривая (логарифмический)")
